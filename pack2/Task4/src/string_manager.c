...
char* string_copy_till_space(char *string, char **result) {
    ...
    Vector_char *resVec = vector_char_create();

    for (; *string != '\0' && !isspace(*string); string++) {
        vector_char_push_back(resVec, *string);
    }

    vector_char_push_back(resVec, '\0');

    *result = vector_char_steal_data(resVec);
    vector_char_delete(resVec);
